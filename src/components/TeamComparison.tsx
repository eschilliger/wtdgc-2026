...
function TeamCard({ team, featured = false, disabledIds, onTogglePlayer, onReset }: {
  ...
  const benchValue = `H ${scenario.menBenchGap ?? "—"} · F ${scenario.womenBenchGap ?? "—"}`;
  ...
  return (
    <article className={`team-card${featured ? " team-card--featured" : ""}`}>
      ...
      <div className="team-card__metrics">
        ...
        <Metric label="Profondeur du banc" value={benchValue} detail="écart 4e→5e H / 2e→3e F" />
      </div>
      ...
      {!scenario.complete ? (
        ...
        <div className="scenario-note"><strong>Classement provisoire du six actif</strong><span>J1 à J6 sont attribués uniquement aux 4 hommes + 2 femmes retenus. Les autres joueurs restent remplaçants jusqu'à leur entrée dans le six.</span></div>
      )}
          ...
          return (
            <div className={`player-row player-row--scenario${selected ? " player-row--selected" : " player-row--substitute"}${disabled ? " player-row--disabled" : ""}`} key={player.id}>
              ...
              <div className="player-row__main">
                ...
                <span>{player.gender === "F" ? "Femme" : "Homme"}{player.genderSource === "default-m" ? "*" : ""}{player.pdgaNumber ? ` · PDGA #${player.pdgaNumber}` : ""}{selected ? " · retenu 4H+2F" : !disabled ? " · remplaçant" : ""}</span>
                <small className="player-impact">{disabled ? "hors scénario" : !selected ? "aucun impact immédiat tant qu'il reste remplaçant" : impact === null ? "absence : composition 4H+2F impossible" : `impact si absent : ${signed(impact)}`}</small>
              </div>
              <div className="player-row__actions">
                <div className="player-row__rating"><strong>{rating ?? "—"}</strong><span>{player.referenceRating != null ? "rating WTDGC" : "PDGA provisoire"}</span></div>
  ...
  return (
    <div className="duel-summary duel-summary--scenario">
      <div className="duel-summary__headline">
        <div><p className="eyebrow">Simulation de composition</p><h3>{gap === null ? "Comparaison incomplète" : leader ? `${flagEmoji(leader.countryCode)} ${leader.country} devant` : "Équilibre actuel"}</h3><p className="duel-summary__copy">J1 à J6 correspondent au six actif, toujours composé de 4 hommes et 2 femmes.</p></div>
  ...
  return (
    <>
      ...
      <section className="comparison-section">
        <div className="section-heading"><div><p className="eyebrow">Scouting</p><h2>Comparer deux équipes</h2><p className="section-copy">Le six actif est toujours construit avec 4 hommes + 2 femmes. Les rangs J1 à J6 sont recalculés après chaque désactivation.</p></div></div>
