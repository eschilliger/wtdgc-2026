...
function Metric({ label, value, detail, className = "" }: { label: string; value: string | number; detail?: string; className?: string }) {
  return <div className={`comparison-metric${className ? ` ${className}` : ""}`}><span>{label}</span><strong>{value}</strong>{detail ? <small>{detail}</small> : null}</div>;
}
...
function TeamScenarioStory({ team, disabledIds }: { team: ComparisonTeam; disabledIds: Set<string> }) {
  ...
  return <div className="scenario-story"><strong>Scénario modifié</strong><div>{exits.length ? <span>Sortie : {exits.map((p) => `${p.firstName} ${p.lastName}`).join(", ")}</span> : null}{entries.length ? <span>Entrée : {entries.map((p) => `${p.firstName} ${p.lastName}`).join(", ")}</span> : null}<span>Rating équipe : {nominal.average ?? "—"} → {scenario.average ?? "—"}{delta !== null ? ` (${signed(delta)})` : ""}</span></div></div>;
}

function DepthSummary({ scenario }: { scenario: ReturnType<typeof lineupSummary> }) {
  return (
    <div className="team-depth">
      <div className="team-depth__item">
        <span>Hommes</span>
        <strong>{depthLabel(scenario.menBenchGap, scenario.menHasBackup)}</strong>
        <small>{scenario.menHasBackup ? `Perte potentielle : -${scenario.menBenchGap} (4e → 5e)` : "Pas de 5e homme disponible"}</small>
      </div>
      <div className="team-depth__item">
        <span>Féminines</span>
        <strong>{depthLabel(scenario.womenBenchGap, scenario.womenHasBackup)}</strong>
        <small>{scenario.womenHasBackup ? `Perte potentielle : -${scenario.womenBenchGap} (2e → 3e)` : "Pas de 3e féminine disponible"}</small>
      </div>
    </div>
  );
}
    ...
    <article className={`team-card${featured ? " team-card--featured" : ""}`}>
      <div className="team-card__header">
        ...
        <div className="team-card__header-actions">
          <div className="rating-summary"><strong>{scenario.average ?? "—"}</strong><span>rating scénario · 4 hommes + 2 féminines</span><small className={delta !== null && delta < 0 ? "scenario-diff scenario-diff--down" : "scenario-diff"}>{nominal.average === null ? "référence incomplète" : `référence ${nominal.average} · ${signed(delta)}`}</small></div>
          <button type="button" className="team-reset" onClick={onReset} disabled={disabledIds.size === 0}>Réinitialiser</button>
        </div>
      </div>

      <div className="team-card__metrics team-card__metrics--v41">
        ...
        <div className="comparison-metric comparison-metric--depth"><span>Profondeur du banc</span><DepthSummary scenario={scenario} /></div>
      </div>
      ...
      {!scenario.complete ? (
        <div className="scenario-alert scenario-alert--error"><strong>Rating équipe impossible à calculer</strong><span>Composition disponible : {scenario.menCount}/4 hommes et {scenario.womenCount}/2 féminines.{scenario.missingRatings.length ? ` Rating manquant pour ${scenario.missingRatings.map((player) => `${player.firstName} ${player.lastName}`).join(", ")}.` : " Il manque au moins un joueur éligible avec un rating exploitable."}</span></div>
      ) : (
        <div className="scenario-note"><strong>Six actif</strong><span>J1 à J6 sont attribués aux 4 hommes + 2 féminines du scénario. Les remplaçants montent automatiquement dans le six en cas de désactivation.</span></div>
      )}

      {scenario.defaultedGender.length ? <div className="scenario-alert scenario-alert--warning"><strong>Genre appliqué par défaut</strong><span>{scenario.defaultedGender.map((player) => `${player.firstName} ${player.lastName}`).join(", ")} : genre PDGA non résolu, classé Homme selon la règle de fallback du scouting.</span></div> : null}
          ...
          return (
            <div className={`player-row player-row--scenario${selected ? " player-row--selected" : " player-row--substitute"}${disabled ? " player-row--disabled" : ""}`} key={player.id}>
              ...
              <div className="player-row__main">
                ...
                <span className="player-meta-line">{player.gender === "F" ? "Féminine" : "Homme"}{player.genderSource === "default-m" ? "*" : ""}{player.pdgaNumber ? ` · PDGA #${player.pdgaNumber}` : ""}</span>
                <span className={`player-status player-status--${statusClass}`}>{statusLabel}</span>
                <small className="player-impact">{disabled ? "hors scénario" : !selected ? "en attente d'une place dans le six" : impact === null ? "absence : composition 4 hommes + 2 féminines impossible" : `impact absence : ${signed(impact)}`}</small>
              ...
              <div className="player-row__actions">
                <div className="player-row__rating"><strong>{refRating ?? "—"}</strong><span>{player.referenceRating != null ? "référence WTDGC" : "PDGA provisoire"}</span>{liveDelta !== null ? <small>PDGA live {player.rating} · {signed(liveDelta)}</small> : null}</div>
...
function LineupMatchups({ teamA, teamB, disabledA, disabledB }: { teamA: ComparisonTeam; teamB: ComparisonTeam; disabledA: Set<string>; disabledB: Set<string> }) {
  ...
  return <div className="lineup-matchups"><div className="lineup-matchups__heading"><div><p className="eyebrow">Lecture du six</p><h3>Joueur par joueur</h3></div><span>Lecture de profondeur uniquement — les Jx ne préjugent pas encore des confrontations de manche.</span></div><div className="lineup-matchups__rows">{a.selected.map((playerA, index) => { const playerB = b.selected[index]; if (!playerB) return null; const ratingA = referenceRating(playerA); const ratingB = referenceRating(playerB); const gap = ratingA !== null && ratingB !== null ? ratingA - ratingB : null; return <div className="lineup-matchup-row" key={`${playerA.id}-${playerB.id}`}><strong>J{index + 1}</strong><div className="lineup-matchup-player lineup-matchup-player--a"><span>{playerA.firstName} {playerA.lastName}</span><b>{ratingA ?? "—"}</b><small>{playerA.trend12Months != null ? `12m ${signed(playerA.trend12Months)}` : ""}</small></div><div className={`lineup-matchup-gap${gap !== null && gap > 0 ? " lineup-matchup-gap--a" : gap !== null && gap < 0 ? " lineup-matchup-gap--b" : ""}`}>{gap === null ? "—" : signed(gap)}</div><div className="lineup-matchup-player lineup-matchup-player--b"><b>{ratingB ?? "—"}</b><span>{playerB.firstName} {playerB.lastName}</span><small>{playerB.trend12Months != null ? `12m ${signed(playerB.trend12Months)}` : ""}</small></div></div>; })}</div></div>;
}
