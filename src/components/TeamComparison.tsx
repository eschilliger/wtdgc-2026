...
function DepthSummary({ scenario }: { scenario: ReturnType<typeof lineupSummary> }) {
  return (
    <div className="team-depth-v43">
      <div><span>Hommes</span><strong>{depthLabel(scenario.menBenchGap, scenario.menHasBackup)}</strong><small>{scenario.menHasBackup ? `4e → 5e : -${scenario.menBenchGap}` : "pas de 5e homme"}</small></div>
      <div><span>Féminines</span><strong>{depthLabel(scenario.womenBenchGap, scenario.womenHasBackup)}</strong><small>{scenario.womenHasBackup ? `2e → 3e : -${scenario.womenBenchGap}` : "pas de 3e féminine"}</small></div>
    </div>
  );
}
    ...
    <div className={`player-row player-row--scenario${selected ? " player-row--selected" : " player-row--substitute"}${disabled ? " player-row--disabled" : ""}`}>
      ...
      <div className="player-row__main">
        <div className="player-name-line">
          {player.pdgaNumber ? <Link className="player-link" href={`/player/${player.pdgaNumber}?division=${division}`}>{player.firstName} {player.lastName}</Link> : <strong>{player.firstName} {player.lastName}</strong>}
      ...
      <div className="player-row__actions">
        ...
        <button type="button" className={`player-toggle${disabled ? " player-toggle--restore" : ""}`} onClick={onToggle}>{disabled ? "Réactiver" : "Désactiver"}</button>
      </div>
    </div>
  );
}
    ...
    <div className="lineup-matchups lineup-matchups--v43">
      ...
      <div className="lineup-matchups__rows">
        {a.selected.map((playerA, index) => {
          ...
          return (
            <div className="lineup-matchup-row lineup-matchup-row--v43" key={`${playerA.id}-${playerB.id}`}>
...
export default function TeamComparison({ teams }: Props) {
  ...
  const switchDivision = (next: "open" | "masters") => { setDivision(next); setTeamAId(""); setTeamBId(""); window.history.replaceState(null, "", `/?division=${next}`); };
